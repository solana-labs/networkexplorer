    ...
    process(inMessage) {
        ...
        if (inMessage.t == "entry") {
            ...
            outMessage.transactions = _.map(inMessage.entry.transactions, (x) => {
                ...
                tx.s = outMessage.s;
                tx.e = outMessage.id;
                tx.signatures = _.map(txn.signatures, (y) => { return { signature: b58e(y.signature), public_key: y.publicKey._bn }; });

                tx.instructions = _.map(txn.instructions, (y) => {
                    let inst = {};

                    inst.keys       = _.map(y.keys, (z) => { return z._bn; });
...
class RedisHandler {
    ...
    process(message, _remote) {
        ...
        if (message.t == "block") {
            ...
            let blkMsg = `${message.h}#${message.l}#${message.s}#${message.dt}#${message.id}`;
            ...
            commands.push(["hmset", `!blk:${message.id}`, {
                t: "blk",
                dt: message.dt,
                h: message.h,
                l: message.l,
        ...
        if (message.t == "entry") {
            ...
            commands.push(["hmset", `!ent:${message.id}`, {
                t: "ent",
                dt: message.dt,
                h: message.h,
                l: message.l,
            ...
            let entMsg = `${message.h}#${message.l}#${message.s}#${message.dt}#${message.id}#${txCount}`;
            ...
            _.forEach(txns, (txn) => {
                ...
                let txnMsg = `${message.h}#${message.l}#${message.s}#${message.dt}#${message.id}#${tx.id}`;
            ...
            this.innerClient.batch(commands).exec((err, results) => {
                ...
                self.innerClient.mget([`!txn-per-sec-max`, `!txn-per-sec:${txn_sec}`], (error, vals) => {
...
if (UDP_ENABLED) {
    ...
    udpServer.on('message', function (data, remote) {
        ...
        let txCount = realMessage.transactions ? realMessage.transactions.length : "<no_tx>";
        ...
        console.log(["!ent", realMessage.dt, realMessage.s, realMessage.h, realMessage.t, realMessage.id, txCount, "took " + (t2 - t1) + "ms"].join(", "));
    });
...
function makeServer() {
    ...
    return net.createServer(function (socket) {
        ...
        socket.on('data', (data) => {
            ...
            console.log(["!ent", pktCount, realMessage.dt, realMessage.s, realMessage.h, realMessage.t, realMessage.id, txCount, "took " + (t2 - t1) + "ms"].join(", "));
        });
    });
}
...
if (TCP_ENABLED) {
    makeServer().listen(config.service.port, config.service.host, () => {
        console.log('TCP Server listening on ' + config.service.host + ":" + config.service.port);
    });
}
