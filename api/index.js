...
app.ws('/', function(ws, req) {
    ...
    console.log((new Date().toISOString()) + ' ws peer [' + ws.my_id + '] connected.');

    ws.on('close', function(reasonCode, description) {
        console.log((new Date().toISOString()) + ' ws peer [' + ws.my_id + '] disconnected: ' + reasonCode + " " + description);
        delete listeners[ws.my_id];
    });
});
...
app.get('/global-stats', (req, res) => {
    let txn_sec = new Date(new Date().getTime() - 3000).toISOString().substring(0,19);
    let stat_keys = [`!global-node-id`, `!blk-last-slot`, `!blk-last-id`, `!txn-per-sec-max`, `!txn-per-sec:${txn_sec}`, `!txn-count`, `!ent-height`, `!ent-last-dt`, `!ent-last-id`];
